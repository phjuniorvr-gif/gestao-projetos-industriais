...
- **`LockBadge`** (`src/components/ui/LockBadge.tsx`) — ícone de cadeado (`lucide-react`, `Lock`) com `title` explicando de quem é a atribuição, cor `text-muted2` (só ícone — respeita a regra de contraste da paleta). Nunca substitui o `disabled` do controle em si, só sinaliza ao lado: quem trava a interação é sempre um `disabled={isAdmin !== true}` direto no `Input`/`Select`/`Button`/`button` nativo, não um wrapper genérico — mais simples de aplicar em campos heterogêneos (texto, select, popover de menu) do que um componente que precisaria clonar/injetar props em filhos arbitrários.
- **`FormField.label` generalizado de `string` pra `ReactNode`** — permite `<FormField label={<>Nome {locked && <LockBadge />}</>}>` sem precisar de um segundo componente só pra isso. Compatível com todo uso anterior (string continua sendo um `ReactNode` válido).
- **Padrão de campo travado**: `disabled` (não `readOnly`) nos controles administrador-only — visual acinzentado nativo do navegador/Tailwind (`disabled:opacity-*`, `disabled:cursor-not-allowed`), consistente em `Input`/`Select`/`Button`/botões de ícone. Campos "Início/Fim real" (informar real, liberado pra qualquer usuário) nunca entram nesse padrão.

## Entregues na Fase 6

- **`BottomSheet`** (`src/components/ui/BottomSheet.tsx`) — primitivo genérico novo (scrim `bg-black/40`, painel `rounded-t-2xl` que entra de baixo com `transition-transform duration-[270ms]`, alça, Esc/clique-fora fecha, trava scroll do body enquanto aberto). Substitui o painel lateral (`fixed inset-0 flex justify-end`) como padrão de overlay no mobile — mesma lógica de fechar, orientação diferente (`items-end` + `translate-y` em vez de vir da direita).
- **`MiniGantt` variante `compact`** (`size="compact"`, 22px) — mesmas duas barras (previsto tracejado, real colorido por status) e marcador de hoje do `default` (26px), sem as guias de 25/50/75% nem a linha de datas embaixo; usada em todo card/sheet mobile, nunca no desktop.
- **`ProjectCard` empilhado** — layout vertical (`space-y-2`), substitui os blocos de largura fixa (`w-40`/`w-48`/`w-32`) que não cabiam em tela estreita; card inteiro é a área de toque (`min-h-11`), abre `MobileProjectSheet` em vez de navegar direto.
